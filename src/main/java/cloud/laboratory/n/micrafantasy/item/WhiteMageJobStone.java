...
public class WhiteMageJobStone extends Item {

    public WhiteMageJobStone(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        if (!level.isClientSide() && player instanceof ServerPlayer serverPlayer) {
            ...
            if (data.getJobType() == JobType.WHITE_MAGE) {
                // 既に白魔道士の場合はジョブを解除
                ...
                // 白魔道士にジョブチェンジ
                data.setJobType(JobType.WHITE_MAGE);
                serverPlayer.displayClientMessage(
                        Component.translatable("message.micrafantasy.job_changed.white_mage"), true);
            }
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, TooltipDisplay tooltipDisplay, Consumer<Component> tooltipAdder, TooltipFlag flag) {
        tooltipAdder.accept(Component.translatable("item.micrafantasy.white_mage_job_stone.tooltip"));
    }
}
